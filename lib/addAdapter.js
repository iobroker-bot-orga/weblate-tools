 ...
 */
async function resolveComponents(entries, adapterName, project) {
    ...
    for (const comp of components) {
        ...
        if (!existing) {
            ...
            comp.flag = '🟡';
            comp.info = 'missing';
        } else if (existing.filemask === comp.fileMask) {
            comp.status = 'valid';
            comp.flag = '🟢';
            comp.info = 'valid';
        } else {
            comp.status = 'invalid';
            comp.flag = '🔴';
            comp.info = `invalid (filemask "${existing.filemask}" != "${comp.fileMask}")`;
        }
    }
...
async function main() {
    ...
    // Step 6: calculate the base component name (identical to adapter name) and
    // the component name/slug of every valid i18n directory, then resolve each
    // against Weblate and log the component table + setup summary (always).
    const adapterName = adapterNameFromRepo(repoName);
    log.info(`Base component name/slug: ${adapterName}`);

    const components = await resolveComponents(entries, adapterName, DEFAULT_PROJECT.slug);
    log.info(`Component report (${components.length} component(s)):`);
    for (const line of formatComponentTable(components)) {
        log.info(line);
    }

    const mainComponent = components.find((c) => c.isMain);
    const willSetUp = mainComponent && mainComponent.status === 'missing' ? [mainComponent] : [];
    const otherMissing = components.filter((c) => !c.isMain && c.status === 'missing');
    log.info('Setup summary:');
    if (willSetUp.length) {
        log.info(`  Will be set up now: ${willSetUp.map((c) => c.name).join(', ')}`);
    } else {
        log.info('  No component will be set up now '
            + `(main component "${mainComponent ? mainComponent.name : adapterName}" `
            + `${mainComponent ? mainComponent.status : 'unknown'}).`);
    }
    if (otherMissing.length) {
        log.info('  Detected but NOT created in this run (only the main component is created currently): '
            + otherMissing.map((c) => c.name).join(', '));
    }
    ...
    // Step 8: abort if the main component already exists. Log the concrete
    // reason, including the existing component's identifying details, so the
    // workflow log makes clear why it stopped.
