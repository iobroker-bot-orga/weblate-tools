...
function detectI18nTrees(tree) {
    ...
    return dirs.map((basePath) => {
        ...
        // ioBroker adapters use the plural "translations.json" in the nested
        // layout; the singular "translation.json" is accepted as a fallback.
        const nestedNames = ['translations.json', 'translation.json'];

        if (paths.has(flatBase)) {
            return { basePath, format: 'flat', fileMask: `${basePath}/*.json`, baseFile: flatBase };
        }
        for (const fileName of nestedNames) {
            const nestedBase = `${basePath}/${DEFAULT_BASE_LANGUAGE}/${fileName}`;
            if (paths.has(nestedBase)) {
                return { basePath, format: 'nested', fileMask: `${basePath}/*/${fileName}`, baseFile: nestedBase };
            }
        }
