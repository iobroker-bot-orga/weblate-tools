 ...
 * add-on when the repository contains the trigger file. Failures on an
 * individual add-on are logged but do not abort the run, so one add-on that is
 * unavailable does not prevent the others (or the rest of the flow).
 *
...
async function installComponentAddons(project, component, treePaths) {
    const addons = [...DEFAULT_ADDONS];

    // ioBroker words.js add-on: only when admin/words.js exists.
    if (treePaths.has(WORDS_TRIGGER_FILE)) {
        if (WORDS_ADDON_NAME) {
            log.info(`"${WORDS_TRIGGER_FILE}" present: adding the ioBroker words.js add-on (${WORDS_ADDON_NAME})`);
            addons.push({ name: WORDS_ADDON_NAME, configuration: {} });
        } else {
            log.warn(`"${WORDS_TRIGGER_FILE}" present but WORDS_ADDON_NAME is not set; `
                + 'skipping the "ioBroker: Save translations into words.js" add-on. '
                + 'Set WORDS_ADDON_NAME to its API identifier to enable it.');
        }
    } else {
        log.info(`"${WORDS_TRIGGER_FILE}" not present: skipping the ioBroker words.js add-on`);
    }
    ...
    for (const addon of addons) {
        try {
            ...
            log.error(`Failed to install add-on "${addon.name}" on component "${component}": ${detail}`);
        }
    }
}
...
async function main() {
    ...
    for (const i18nTree of treesToCreate) {
        ...
        // Step 10: install the standard add-ons (and the ioBroker words.js
        // add-on when admin/words.js is present).
        await installComponentAddons(spec.project, created.slug, treePaths);

        // Step 11: trigger a complete pull of the component from upstream.
