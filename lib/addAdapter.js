...
async function main() {
    ...
    // Two directories must never map to the same component name/slug. If they
    // do, the naming is ambiguous and the run cannot continue safely.
    ...
    if (duplicateNames.length) {
        for (const name of duplicateNames) {
            const dirs = components.filter((c) => c.name === name).map((c) => c.dir).join(', ');
            log.error(`Component name "${name}" is used by more than one i18n directory (${dirs}); aborting.`);
        }
        process.exit(1);
        return;
    }
    ...
    let mismatched = findMismatchedComponents(projectComponents, htmlUrl, expectedNames);
    ...
    for (const comp of components) {
        ...
        if (match) {
            ...
            comp.flag = '🟠';
            comp.info = `to be renamed from ${match.slug}`;
    ...
    if (renames.length) {
        log.info(`  To be renamed: ${renames.map((c) => `${c.renameFrom} -> ${c.name}`).join(', ')}`);
    }
    if (toFix.length) {
    ...
    for (const comp of ordered) {
        ...
        if (comp.status === 'rename') {
            ...
            const patch = { slug: comp.name, name: comp.name };
            ...
            log.info(`Renaming component "${comp.renameFrom}" to "${comp.name}"${patch.filemask ? ' (and correcting file mask)' : ''}`);
            await updateComponent(DEFAULT_PROJECT.slug, comp.renameFrom, patch);
            changes.push(`component ${comp.renameFrom} renamed to ${comp.name}`);
            if (patch.filemask) {
            ...
            await ensureAddons(DEFAULT_PROJECT.slug, comp.name, expected, changes);
            continue;
        }
