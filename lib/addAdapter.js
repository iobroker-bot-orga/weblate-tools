 ...
 * @param {string} displayName - the component display name (for change lines).
 ...
 */
async function ensureAddons(project, component, displayName, expected, changes, knownAddons) {
    ...
    for (const addon of expected) {
        ...
        try {
            ...
            changes.push(`component ${component} ("${displayName}"): add-on ${addon.name} added`);
        } catch (error) {
 ...
 */
async function resolveComponents(entries, adapterName, project, expected, license) {
    const expectedAddonNames = new Set(expected.map((a) => a.name));
    const expectedLicense = license ? license.spdx : null;

    const components = entries
        .filter((e) => e.valid)
        .map((e) => ({
            slug: componentSlugFor(adapterName, e.path, e.isMain),
            name: componentDisplayName(adapterName, e.path),
            dir: e.path,
            format: e.format,
            fileMask: e.fileMask,
            baseFile: e.baseFile,
    ...
    for (const comp of components) {
        const existing = await getComponent(project, comp.slug);
        ...
        // Verify the component (display) name.
        if (existing.name !== comp.name) {
            comp.problems.push(`name is "${existing.name}", expected "${comp.name}"`);
        }
        ...
        // Verify license (only when the repository license was detected).
        if (expectedLicense && (existing.license || '') !== expectedLicense) {
            comp.problems.push(`license is "${existing.license || ''}", expected "${expectedLicense}"`);
        }
        ...
        for (const name of expectedAddonNames) {
            if (!have.has(name)) {
    ...
    for (const comp of projectComponents) {
        ...
        if (normalize(effectiveRepo(comp)) === target) {
            mismatched.push({
                slug: comp.slug,
                name: comp.name,
                dir: dirFromFileMask(comp.filemask),
                filemask: comp.filemask,
...
function formatComponentTable(components) {
    const rows = [...components].sort((a, b) => a.slug.localeCompare(b.slug, 'en'));

    const slugHeader = 'Slug';
    const nameHeader = 'Name';
    const dirHeader = 'i18n directory';
    const slugW = Math.max(slugHeader.length, ...rows.map((r) => r.slug.length));
    ...
    const lines = [];
    lines.push(`   ${slugHeader.padEnd(slugW)}  ${nameHeader.padEnd(nameW)}  ${dirHeader.padEnd(dirW)}  Info`);
    for (const r of rows) {
        lines.push(`${r.flag} ${r.slug.padEnd(slugW)}  ${r.name.padEnd(nameW)}  ${r.dir.padEnd(dirW)}  ${r.info}`);
    }
    return lines;
}

/**
 * Render the detected configuration problems (slug + name + problem) as a table.
 *
 * @param {Array<object>} components - from `resolveComponents`.
...
function formatProblemTable(components) {
    const rows = [];
    for (const comp of [...components].sort((a, b) => a.slug.localeCompare(b.slug, 'en'))) {
        for (const problem of comp.problems) {
            rows.push({ slug: comp.slug, name: comp.name, problem });
        }
    }
    if (rows.length === 0) {
        return [];
    ...
    const slugW = Math.max(slugHeader.length, ...rows.map((r) => r.slug.length));
    const nameW = Math.max(nameHeader.length, ...rows.map((r) => r.name.length));
    const lines = [`${slugHeader.padEnd(slugW)}  ${nameHeader.padEnd(nameW)}  Problem detected`];
    for (const r of rows) {
        lines.push(`${r.slug.padEnd(slugW)}  ${r.name.padEnd(nameW)}  ${r.problem}`);
    }
    return lines;
}
 ...
 *
 * @param {Array<{ slug: string, name: string, dir: string }>} mismatched
...
function formatMismatchTable(mismatched) {
    ...
    const slugW = Math.max(slugHeader.length, ...rows.map((r) => r.slug.length));
    const nameW = Math.max(nameHeader.length, ...rows.map((r) => (r.name || '').length));
    const lines = [`${slugHeader.padEnd(slugW)}  ${nameHeader.padEnd(nameW)}  Related directory`];
    for (const r of rows) {
        lines.push(`${r.slug.padEnd(slugW)}  ${(r.name || '').padEnd(nameW)}  ${r.dir || '(unknown)'}`);
    }
    return lines;
}

/** URL of a component in the Weblate web UI. */
function componentUrl(slug) {
    return `${WEBLATE_URL.replace(/\/+$/, '')}/projects/${DEFAULT_PROJECT.slug}/${slug}/`;
}

/**
 * Build the title + body of the GitHub issue posted to the adapter repository
 * after components were added and/or modified.
 *
 * @param {Array<object>} allComponents - every component of this repository
 *        (existing + added), used for the component links.
 * @param {Array<object>} added - newly created components.
 * @param {Array<object>} modified - existing components that were changed.
 * @param {string[]} changes - the detailed change log.
 * @returns {{ title: string, body: string }}
 */
function buildAdapterIssue(allComponents, added, modified, changes) {
    const title = added.length && modified.length
        ? '[weblate] Repository added to and modified at weblate'
        : added.length
            ? '[weblate] Repository added to weblate'
            : '[weblate] Repository modified at weblate';

    // Links to ALL components of this repository (existing and added).
    const links = [...allComponents]
        .sort((a, b) => a.slug.localeCompare(b.slug, 'en'))
        .map((c) => `- [${c.name}](${componentUrl(c.slug)})`);
    const summary = changes.map((c) => `- ${c}`);

    // English version.
    const en = [];
    if (added.length) {
        en.push(
            'Some components have been added for this repository to weblate (https://weblate.iobroker.net).',
            '',
            'You can reach the component(s) here:',
            ...links,
            '',
            'Please add the badge for Weblate translations to your adapter repository if not already done:',
            'https://github.com/ioBrokerTranslator/doc#add-badge-to-readmemd',
            '',
            'Please check all translations of all languages known by you, correct translations and remove the '
                + '"needs editing" flag for those translations checked by you.',
            '',
        );
    } else if (modified.length) {
        en.push(
            'Some components have been modified at weblate (https://weblate.iobroker.net). See details below.',
            '',
            'You can reach the component(s) here:',
            ...links,
            '',
    ...
    en.push(
        'From now on, please follow these guidelines:',
        'https://github.com/ioBrokerTranslator/doc#maintenance-of-your-adapter',
        '',
        '## Summary',
        ...summary,
        '',
    ...
    if (added.length) {
        de.push(
            'Für dieses Repository wurden Komponenten zu Weblate (https://weblate.iobroker.net) hinzugefügt.',
            '',
            'Du erreichst die Komponente(n) hier:',
            ...links,
            '',
            'Bitte füge das Badge für Weblate-Übersetzungen zu deinem Adapter-Repository hinzu, falls noch nicht geschehen:',
            'https://github.com/ioBrokerTranslator/doc#add-badge-to-readmemd',
            '',
            'Bitte überprüfe alle Übersetzungen aller dir bekannten Sprachen, korrigiere die Übersetzungen und '
                + 'entferne die Markierung "needs editing" für die von dir geprüften Übersetzungen.',
            '',
        );
    } else if (modified.length) {
        de.push(
            'Einige Komponenten wurden bei Weblate (https://weblate.iobroker.net) geändert. Details siehe unten.',
            '',
            'Du erreichst die Komponente(n) hier:',
            ...links,
            '',
    ...
    de.push(
        'Bitte befolge ab jetzt diese Richtlinien:',
        'https://github.com/ioBrokerTranslator/doc#maintenance-of-your-adapter',
        '',
        '## Zusammenfassung',
        ...summary,
        '',
        'Kontaktiere mich (@ioBroker-Bot) gerne, falls du Fragen hast.',
        '',
        'Und VIELEN DANK, dass du diesen Adapter pflegst – von mir und allen Nutzern. '
            + 'Lass uns für das beste Nutzererlebnis zusammenarbeiten.',
        '',
        'dein',
        'ioBroker Check and Service Bot',
        '',
    ...
    const body = [
        'A german version is located below | Eine deutsche Version befindet sich weiter unten',
        '',
        ...en,
        '',
        '---',
...
async function main() {
    ...
    // Step 6: compute the base slug and, for every valid i18n directory, the
    // component slug/name; then resolve each against Weblate (verifying file
    // mask, VCS, add-ons, display name and license) and log the reports
    // (always). The license is extracted here (a read-only GitHub call) so it
    // can be verified even in precheck mode.
    ...
    if (!license) {
        log.info('License could not be determined; the license will not be verified or set');
    }

    const expected = expectedAddons(treePaths);
    const components = await resolveComponents(entries, adapterName, DEFAULT_PROJECT.slug, expected, license);

    // Two directories must never map to the same component slug. If they do,
    // the naming is ambiguous and the run cannot continue safely.
    ...
    if (duplicateSlugs.length) {
        for (const slug of duplicateSlugs) {
            const dirs = components.filter((c) => c.slug === slug).map((c) => c.dir).join(', ');
            log.error(`Component slug "${slug}" is used by more than one i18n directory (${dirs}); aborting.`);
        }
        process.exit(1);
        return;
    }

    // Components already linked to this repository whose slug does not match the
    // expected set (misnamed / stale).
    const expectedSlugs = new Set(components.map((c) => c.slug));
    const projectComponents = await getProjectComponents(DEFAULT_PROJECT.slug);
    let mismatched = findMismatchedComponents(projectComponents, htmlUrl, expectedSlugs);

    // A missing component whose i18n directory matches a mismatched (wrongly
    // named) component is satisfied by RENAMING that component rather than
    // creating a new one. Such a mismatched component is consumed here.
    const consumed = new Set();
    for (const comp of components) {
        if (comp.status !== 'missing') {
            continue;
        }
        const match = mismatched.find((m) => m.dir === comp.dir && !consumed.has(m.slug));
        if (match) {
            comp.status = 'rename';
            comp.renameFrom = match.slug;
            comp.renameFromName = match.name;
            comp.renameFromFilemask = match.filemask;
            comp.renameFromLicense = match.license;
            comp.flag = '🟠';
            comp.info = `to be renamed from ${match.slug} ("${match.name}")`;
            consumed.add(match.slug);
        }
    }
    mismatched = mismatched.filter((m) => !consumed.has(m.slug));
    ...
    const renames = components.filter((c) => c.status === 'rename');
    const toFix = components.filter((c) => c.status === 'invalid');
    log.info('');
    const describe = (c) => `${c.slug} ("${c.name}")`;
    log.info('Setup summary:');
    log.info(missing.length
        ? `  To be created: ${missing.map(describe).join(', ')}`
        : '  No components need to be created.');
    if (renames.length) {
        log.info(`  To be renamed: ${renames.map((c) => `${c.renameFrom} -> ${describe(c)}`).join(', ')}`);
    }
    if (toFix.length) {
        log.info(`  To be fixed: ${toFix.map(describe).join(', ')}`);
    }
    ...
    // Step 8: repository browser URL (shared by all components). The license
    // was already extracted (Step 6) for verification.
    ...
    // ones (file mask, name, VCS, license, add-ons). The main must exist before
    // linked components can reference it, so it is processed first.
    const changes = [];
    ...
    const ordered = [
        mainComp,
        ...components.filter((c) => !c.isMain).sort((a, b) => a.slug.localeCompare(b.slug, 'en')),
    ].filter(Boolean);

    for (const comp of ordered) {
        if (comp.status === 'missing') {
            const spec = buildComponentSpec({ comp, repoUrl: htmlUrl, branch, mainSlug: adapterName, repoweb, license });
            log.info(`Creating component ${comp.slug} ("${comp.name}") (repo: ${spec.repo}`
                + `${spec.vcs ? `, vcs: ${spec.vcs}` : ' [linked]'}, mask: ${spec.fileMask})`);
            const created = await createComponent(spec);
            changes.push(`component ${comp.slug} ("${comp.name}") added`);
            createdComponents.push(comp);
            await ensureAddons(DEFAULT_PROJECT.slug, comp.slug, comp.name, expected, changes, await listComponentAddons(created));
            continue;
        }

        if (comp.status === 'rename') {
            // Rename the wrongly-named component to the expected slug/name, also
            // correcting its file mask and license in the same request when needed.
            const patch = { slug: comp.slug, name: comp.name };
            if (comp.renameFromFilemask !== comp.fileMask) {
                patch.filemask = comp.fileMask;
                patch.template = comp.baseFile;
                patch.new_base = comp.baseFile;
            }
            if (license && (comp.renameFromLicense || '') !== license.spdx) {
                patch.license = license.spdx;
            }
            log.info(`Renaming component ${comp.renameFrom} ("${comp.renameFromName}") to ${comp.slug} ("${comp.name}")`
                + `${patch.filemask ? ' (and correcting file mask)' : ''}`);
            await updateComponent(DEFAULT_PROJECT.slug, comp.renameFrom, patch);
            changes.push(`component ${comp.renameFrom} renamed to ${comp.slug} ("${comp.name}")`);
            if (patch.filemask) {
                changes.push(`component ${comp.slug} ("${comp.name}") filemask changed to ${patch.filemask}`);
            }
            if (patch.license) {
                changes.push(`component ${comp.slug} ("${comp.name}") license changed to ${patch.license}`);
            }
            // Add any missing add-ons on the (now correctly named) component.
            await ensureAddons(DEFAULT_PROJECT.slug, comp.slug, comp.name, expected, changes);
            modifiedComponents.push(comp);
            continue;
        }

        // Existing component: correct file mask / name / VCS / license, then add
        // missing add-ons.
        ...
        if (comp.existing.name !== comp.name) {
            patch.name = comp.name;
        }
        if (comp.isMain && comp.existing.vcs !== DEFAULT_VCS) {
            patch.vcs = DEFAULT_VCS;
        }
        if (license && (comp.existing.license || '') !== license.spdx) {
            patch.license = license.spdx;
        }
        if (Object.keys(patch).length) {
            log.info(`Fixing component ${comp.slug} ("${comp.name}"): ${JSON.stringify(patch)}`);
            await updateComponent(DEFAULT_PROJECT.slug, comp.slug, patch);
            if (patch.filemask) {
                changes.push(`component ${comp.slug} ("${comp.name}") filemask changed to ${patch.filemask}`);
            }
            if (patch.name) {
                changes.push(`component ${comp.slug} name changed to "${patch.name}"`);
            }
            if (patch.vcs) {
                changes.push(`component ${comp.slug} ("${comp.name}") vcs changed to ${patch.vcs}`);
            }
            if (patch.license) {
                changes.push(`component ${comp.slug} ("${comp.name}") license changed to ${patch.license}`);
            }
        }
        await ensureAddons(DEFAULT_PROJECT.slug, comp.slug, comp.name, expected, changes, comp.existingAddons);
    ...
    // as requested.
    for (const comp of ordered) {
        log.info(`Triggering a complete pull of component ${comp.slug} ("${comp.name}")`);
        await pullComponentRepository(DEFAULT_PROJECT.slug, comp.slug);
    }
    ...
    for (const comp of createdComponents) {
        ...
        log.info(`Marking non-"${DEFAULT_BASE_LANGUAGE}" translations of ${comp.slug} ("${comp.name}") as needs editing `
            + `(expecting ${expectedLanguages} language(s))`);
        const summary = await markComponentNeedsEditing(DEFAULT_PROJECT.slug, comp.slug, {
            baseLanguage: DEFAULT_BASE_LANGUAGE,
            expectedLanguages,
        });
        log.info(`  ${comp.slug} ("${comp.name}"): base cleared ${summary.baseCleared}, `
            + `${summary.updated} set to needs editing, ${summary.skipped} skipped, ${summary.failed} failed`);
        if (summary.failed > 0) {
            throw new Error(`${summary.failed} string(s) failed to update to "needs editing" for component ${comp.slug}`);
        }
    }
    ...
    // Step 13: open an issue on the adapter repository summarising the changes
    // (as the ioBroker bot, via IOBBOT_GITHUB_TOKEN). Only when something
    // actually changed. A failure here does not undo the Weblate work.
    if (createdComponents.length || modifiedComponents.length) {
        if (!getEnv('IOBBOT_GITHUB_TOKEN')) {
            log.warn('IOBBOT_GITHUB_TOKEN is not set; skipping creation of the summary issue '
                + `on ${owner}/${repoName}.`);
        } else {
            const { title, body } = buildAdapterIssue(components, createdComponents, modifiedComponents, changes);
            try {
                log.info(`Creating summary issue on ${owner}/${repoName}: "${title}"`);
                const issue = await createIssue(owner, repoName, title, body);
                log.info(`Issue created: ${issue.html_url || `#${issue.number}`}`);
            } catch (error) {
                const detail = error.response && error.response.data ? JSON.stringify(error.response.data) : error.message;
                log.error(`Failed to create the summary issue on ${owner}/${repoName}: ${detail}`);
            }
        }
    }
