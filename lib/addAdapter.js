 ...
 */
function buildAdapterIssue(added, modified, changes) {
    ...
    const links = added.map((c) => `- [${c.name}](${componentUrl(c.slug)})`);
...
async function main() {
    ...
    if (createdComponents.length || modifiedComponents.length) {
        if (!getEnv('IOBBOT_GITHUB_TOKEN')) {
            ...
            const { title, body } = buildAdapterIssue(createdComponents, modifiedComponents, changes);
            try {
