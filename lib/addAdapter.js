...
async function resolveComponents(entries, adapterName, project, expected) {
    ...
    for (const comp of components) {
        ...
        comp.existingAddons = await getComponentAddons(project, comp.name);
...
async function main() {
    ...
    for (const comp of ordered) {
        if (comp.status === 'missing') {
            ...
            await ensureAddons(DEFAULT_PROJECT.slug, created.slug, expected, changes);
            continue;
        }
    ...
    // Step 10: for newly created components, pull the repository and bulk-mark
    // non-English translations as "needs editing". The pull on the main loads
    // the shared checkout used by the linked components too.
    if (createdComponents.length) {
        log.info(`Triggering a complete pull of component "${adapterName}"`);
        await pullComponentRepository(DEFAULT_PROJECT.slug, adapterName);
    ...
    // Step 11: final report of all changes done.
    if (changes.length) {
