...
/**
 * Count the language files present for a detected i18n tree, i.e. the number of
 * translations Weblate should end up creating for it. Used to wait until the
 * fresh component has finished processing.
 *
 * @param {Array<{ path: string, type: string }>} tree - recursive git tree.
 * @param {object} i18nTree - a descriptor from `detectI18nTrees`.
...
async function main() {
    ...
    for (const i18nTree of treesToCreate) {
        ...
        // Step 11: trigger a complete pull of the component from upstream so
        ...
        // Step 12: bulk-mark every non-English translation as "needs editing".
