...
async function evaluateI18nTrees({ tree, owner, repo, branch }) {
    ...
    if (adminI18n && adminI18n.valid) {
        ...
        // Prefer src-admin/i18n; if it does not exist (or is not valid), fall
        // back to src-admin/src/i18n.
        const srcAdmin = byPath('src-admin/i18n');
        const srcAdminSrc = byPath('src-admin/src/i18n');
        if (srcAdmin && srcAdmin.valid) {
            main = srcAdmin;
        } else if (srcAdminSrc && srcAdminSrc.valid) {
            main = srcAdminSrc;
        }
    }
    if (main) {
...
/**
 * Calculate the Weblate component **slug** for an i18n directory.
 *
 * The main directory maps to the base slug (the adapter name). Every other
 * directory becomes `<adapterName>_<dir>` where `<dir>` is the directory path
 * without the trailing `i18n` segment and with `/` replaced by `_`.
 * e.g. adapter `foo`, dir `src-admin/src/i18n` → `foo_src-admin_src`.
 *
 * @param {string} adapterName - the base slug (adapter name).
 * @param {string} dirPath - the i18n directory path.
 * @param {boolean} isMain - whether this is the main directory.
 * @returns {string} the component slug.
 */
function componentSlugFor(adapterName, dirPath, isMain) {
    if (isMain) {
        return adapterName;
    }
...
/**
 * Calculate the Weblate component **display name** for an i18n directory:
 * the adapter name followed by the directory (with a leading slash) in
 * parentheses. e.g. adapter `shelly`, dir `admin/i18n` →
 * `shelly (/admin/i18n)`.
 *
 * @param {string} adapterName - the base name (adapter name).
 * @param {string} dirPath - the i18n directory path.
 * @returns {string} the component display name.
 */
function componentDisplayName(adapterName, dirPath) {
    return `${adapterName} (/${dirPath})`;
}
