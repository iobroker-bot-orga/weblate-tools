...
function componentNameFor(adapterName, dirPath, isMain) {
    ...
    // Strip the trailing "i18n" segment. For an i18n directory at the repo root
    // this leaves nothing, so fall back to "i18n" as the suffix — never the bare
    // adapter name, which would collide with the main component.
    const stripped = dirPath.replace(/(^|\/)i18n$/, '').replace(/^\/+|\/+$/g, '');
    const suffix = stripped || 'i18n';
    return `${adapterName}_${suffix.replace(/\//g, '_')}`;
}
