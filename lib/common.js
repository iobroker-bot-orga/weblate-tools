...
/**
 * Map over items applying an async function with bounded concurrency, so a
 * large number of (potentially slow) requests does not all fire at once.
 * Results preserve the order of the input items.
 *
 * @template T, R
 * @param {T[]} items
 * @param {number} limit - maximum number of concurrent invocations.
