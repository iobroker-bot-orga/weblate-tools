...
/**
 * Maximum number of Weblate requests in flight at once (global). The slow
 * server must not be hit with more than this many parallel connections.
 * Overridable via WEBLATE_MAX_CONCURRENCY.
 */
const WEBLATE_MAX_CONCURRENCY = Number(process.env.WEBLATE_MAX_CONCURRENCY) > 0
    ? Number(process.env.WEBLATE_MAX_CONCURRENCY)
    : 5;

/**
 * A minimal counting semaphore: `acquire()` resolves once a slot is free (up to
 * `max` concurrent holders); `release()` frees a slot and wakes the next waiter.
 *
 * @param {number} max
...
/** Global limiter shared by every Weblate request (see WEBLATE_MAX_CONCURRENCY). */
const weblateGate = createSemaphore(WEBLATE_MAX_CONCURRENCY);

/**
 * Delays (ms) before retrying a Weblate request that timed out: 1, 2 then 5
 * minutes (so up to 3 retries).
 */
const TIMEOUT_RETRY_DELAYS_MS = [60000, 120000, 300000];

/** Extra wait added on top of a throttling `Retry-After`, and the retry cap. */
const THROTTLE_EXTRA_MS = 30000;
const MAX_THROTTLE_RETRIES = 5;

/** Whether an axios error is a request timeout (as opposed to an HTTP status). */
function isTimeoutError(error) {
    ...
    return !error.response && /timeout/i.test(error.message || '');
}

/**
 * Milliseconds to wait for a throttled (HTTP 429) response, from the
 * `Retry-After` header (seconds or an HTTP date). Falls back to 60 s.
 *
 ...
 * debug level and mask any sensitive data. Timeouts are retried up to 3 times
 * (after 1/2/5 minutes) and throttling (HTTP 429) is retried after the
 * suggested delay plus 30 s.
 *
...
function getWeblateClient() {
    ...
    client.interceptors.request.use(async (requestConfig) => {
        // Global limiter: never keep more than WEBLATE_MAX_CONCURRENCY requests
        // in flight. The slot is released in the response/error interceptors.
        await weblateGate.acquire();
        requestConfig.__slotHeld = true;
    ...
    // Release the concurrency slot held by a request config (idempotent).
    const releaseSlot = (config) => {
        if (config && config.__slotHeld) {
            config.__slotHeld = false;
            weblateGate.release();
        }
    };
    ...
    client.interceptors.response.use(
        ...
        async (error) => {
            ...
            // Free the slot immediately so a retry wait (below) does not hold a
            // concurrency slot; the retry re-acquires via the request interceptor.
            releaseSlot(config);

            // Throttling: wait the suggested Retry-After plus 30 s, then retry.
            if (error.response && error.response.status === 429) {
                config.__throttleRetries = (config.__throttleRetries || 0) + 1;
                if (config.__throttleRetries <= MAX_THROTTLE_RETRIES) {
                    const waitMs = retryAfterMs(error.response) + THROTTLE_EXTRA_MS;
                    log.warn(`Weblate throttled (429) on ${url}; waiting ${Math.round(waitMs / 1000)}s before `
                        + `retry ${config.__throttleRetries}/${MAX_THROTTLE_RETRIES}`);
                    await sleep(waitMs);
                    return client(config);
                }
            }

            // Timeout: retry up to 3 times after 1 / 2 / 5 minutes.
            if (isTimeoutError(error)) {
                config.__timeoutRetries = (config.__timeoutRetries || 0) + 1;
                if (config.__timeoutRetries <= TIMEOUT_RETRY_DELAYS_MS.length) {
                    const waitMs = TIMEOUT_RETRY_DELAYS_MS[config.__timeoutRetries - 1];
                    log.warn(`Weblate request to ${url} timed out; `
                        + `retry ${config.__timeoutRetries}/${TIMEOUT_RETRY_DELAYS_MS.length} in ${Math.round(waitMs / 1000)}s`);
                    await sleep(waitMs);
                    return client(config);
                }
                log.error(`Weblate request to ${url} timed out after ${TIMEOUT_RETRY_DELAYS_MS.length} retries`);
            }
