...
/**
 * Determine whether a component is locked. Prefers the `locked` field carried
 * by the component object; if that field is absent, falls back to querying the
 * authoritative lock endpoint.
 *
...
async function isComponentLocked(component) {
    ...
    log.debug(`Component "${component.slug}" has no "locked" field; querying lock endpoint`);
