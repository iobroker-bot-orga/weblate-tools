...
const { listProjectComponents, isComponentLocked } = require('./weblateTools');
...
async function main() {
    ...
    // Resolve the lock state authoritatively (falls back to the lock endpoint
    // when a component object does not carry a reliable `locked` field).
    const lockStates = await Promise.all(sorted.map((component) => isComponentLocked(component)));
