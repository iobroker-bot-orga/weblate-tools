...
async function main() {
    ...
    // ignored. Components are processed sequentially in alphabetical order so
    // progress is easy to follow and the (slow) server is not overloaded.
    log.debug(`Resolving lock status for ${sorted.length} component(s) via the lock endpoint`);
    const lockStates = [];
    for (const component of sorted) {
        log.info(`Processing ${component.name} [slug: ${component.slug}]`);
        lockStates.push(await getComponentLockStatus(project, component.slug));
    }
