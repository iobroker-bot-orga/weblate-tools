...
/**
 * Maximum number of parallel Weblate requests for `mapWithConcurrency`-based
 * batches (lock checks, unit updates). The slow server must not be hit with
 * more than this many concurrent requests. Overridable via
 * WEBLATE_MAX_CONCURRENCY.
 */
const WEBLATE_MAX_CONCURRENCY = Number(process.env.WEBLATE_MAX_CONCURRENCY) > 0
    ? Math.min(5, Number(process.env.WEBLATE_MAX_CONCURRENCY))
    : 5;
 ...
 * add-on. The known identifier on weblate.iobroker.net is used as the default
 * and can be overridden via the WORDS_ADDON_NAME environment variable (to
 * retrieve it from an existing component: GET /api/components/adapters/<slug>/addons/).
 */
const WORDS_ADDON_NAME = process.env.WORDS_ADDON_NAME || 'iobroker.weblate.gulp.adminLanguages2words';
